...
class RecommendMassagerViewHolder(itemView: View) : AbstractBaseViewHolder(itemView) {
    ...
    fun bind(massger: RecommendMassager) {
        ...
        Glide.with(getContext()).load(massger.imageUrl).apply(RequestOptions().circleCrop()).into(itemView.imv_massager)
        ...
        val isFreeCancel = if (massger.isFreeCancel) {
            getString(R.string.common_free_cancel)
        } else {
            ""
        }
